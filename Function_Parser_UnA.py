...
def store_external_dictionary(string, path, machine_id, sub_id):
    site_text = string
    machine_id = machine_id
    path = path
    list_of_urls = get_urls(site_text, machine_id, sub_id)
...
def get_urls(site_text, machine_id, sub_id):
    ...
    for url in pre_processed_urls:
            ...
            list_of_urls.append(process_url(url, machine_id, sub_id))
...
def process_url(url, machine_id, sub_id):
    url = str(url)
    '''
    ...
    url = url[0] + "//" + url[2] + "/"
    '''
    with open("term_output_" + machine_id + "_" + sub_id + ".txt", 'a') as f:
        f.write(url + "\n")
    print(url)
    return(url)
